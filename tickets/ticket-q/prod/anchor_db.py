...
from anchor_ext import TicketQMixin


class AnchorDB(TicketQMixin):
    """SQLite storage with graph layer for memory synapses.

    TICKET-Q（2026-09-21）：Q1–Q4 的儲存層擴充在 anchor_ext.TicketQMixin，migration 由 _init_tables 末尾呼叫。
    """
    ...
    def _init_tables(self):
        ...
        self._migrate_ticketq()  # TICKET-Q：冪等 migration + 污染期快照
    ...
    def annotate(self, memory_id: str, text: str) -> int:
        ...
        self.reinforce(memory_id, reason="annotate")  # TICKET-Q：cn 明確動作 → 遞減強化
    ...
    def insert(self, memory_id: str, text: str, tag: str = "general",
               ...
               context: str = "", kind: str = "memory", derived_from: list = None):
        """Insert or update a memory. text = search summary, context = full original.

        TICKET-Q：改 UPSERT，不再 INSERT OR REPLACE——REPLACE 是 DELETE+INSERT，會經 ON DELETE CASCADE
        清掉該記憶的 edges / comments / annotations，並把 pinned / reinforcement 歸零。
        同 id 覆蓋（跨窗近況卡每天做）只更新內容欄，保留圖與強化訊號。
        kind / derived_from（Q4）：derived 類（daily_digest 等）derived_from 必填，並寫 memory_lineage。
        """
        self._ensure_context_column()
        sources = self.validate_kind(kind, derived_from)
        kind = kind or "memory"
        df_json = json.dumps(sources, ensure_ascii=False) if sources else None
        with self._conn() as conn:
            conn.execute(
                "INSERT INTO memories (memory_id, text, timestamp, tag, tier, emotion_score, context, kind, derived_from) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) "
                "ON CONFLICT(memory_id) DO UPDATE SET text=excluded.text, timestamp=excluded.timestamp, "
                "tag=excluded.tag, tier=excluded.tier, emotion_score=excluded.emotion_score, "
                "context=excluded.context, kind=excluded.kind, derived_from=excluded.derived_from",
                (memory_id, text, datetime.utcnow().isoformat(), tag, tier, emotion_score, context, kind, df_json),
            )
            self._write_lineage(conn, memory_id, sources)
            conn.commit()
        self.log_event(memory_id, "created",
                       f"tag={tag} tier={tier} kind={kind}" + (f" derived_from={len(sources)}" if sources else ""))
    ...
    def cite(self, memory_id: str):
        """cn 明確引用 → reinforce（遞減封頂）。

        TICKET-Q：不再動 usage_count。那個欄位是污染期（search 每次回傳自動 +1）的 legacy 計數，
        凍結留檔（快照在 legacy_usage_count；污染期 ≤ schema_meta.pollution_cutoff）。
        排序的 citation_boost 沿用凍結值直到 P1b 換 ranker——P1a 不碰排序公式。
        """
        return self.reinforce(memory_id, reason="cite")
    ...
    def connect_batch(self, pairs: list, weight: float = 0.2):
        """Batch connect pairs of memories (legacy Hebbian, 無遞減、無封頂以外的限制).

        TICKET-Q：search() 與 consolidate() 都不再呼叫這個；passive 強化改走 reinforce_pairs()（遞減）。
        保留給外部工具相容。"""
        now = datetime.utcnow().isoformat()
        with self._conn() as conn:
