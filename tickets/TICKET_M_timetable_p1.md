...
3. **前端**(照打樣):
   - 行程鏡頭上方「月／週」切換;週視圖(時間列 × 一–五,週末有課才顯示欄;**時間軸 = 學期所有 slot 最早開始→最晚結束、對齊節次,不寫死**;今天欄提亮;現在時刻線;停課斜紋;整天 holiday 灰;自主學習週淡化)。
...
4. **測試**(先破後立,每條寫明拿掉什麼要紅):
   ...
   - 學期:last_class_date 之後 week 端點回 `self_study=true`;week 端點回的 `axis_start/axis_end` 等於 slots 最早/最晚(先破後立:多加一門早上 8 點的課,axis_start 要變)。
