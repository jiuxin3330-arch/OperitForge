...
規格正本:`plans/PLAN_timetable_20260908.md`(v1 + v2 + v3,衝突以 v3 為準)。打樣:artifact 9f494bad(糯糯 9/14 蓋章,不用改)。

## 範圍(P1,只碰 chatnest-next 前端與後端;不碰 bridge、prompt、工具說明書、cn 端點)

1. **資料層**(migration 加表加欄,帶預設值,舊資料與既有 schedule 端點不受影響):
   `timetable_terms`、`timetable_courses`、`timetable_slots`、`timetable_overrides`,`schedule_events` 加 `kind`、`course_id`、`slot_id`、`course_name_snapshot`。外鍵行為照 v3 R5。
...
3. **前端**(照打樣):
   - 行程鏡頭上方「月／週」切換;週視圖(時間列 × 一–五,週末有課才顯示欄;今天欄提亮;現在時刻線;停課斜紋;整天 holiday 灰;自主學習週淡化)。
   ...
   - 編輯課表:學期(開學、學期結束、最後上課日;16+2 / 18 快速選項只算日期)、課程列表、新增課程(課名、星期多選、開始/結束、節次(選填)、教室、老師、顏色)。
...
4. **測試**(先破後立,每條寫明拿掉什麼要紅):
   ...
   - 學期:last_class_date 之後 week 端點回 `self_study=true`。
...
- 盲測 9/15 結束,但 P1 仍**不碰** bridge/prompt/工具描述;那是 P2 的單。
