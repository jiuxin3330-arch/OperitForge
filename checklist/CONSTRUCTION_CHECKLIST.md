...
- **注入預算制:隱藏脈絡的總量上限不因新功能增加。** 每個要往 prompt/隱藏脈絡加東西的功能,
  工單必須附三欄注入帳——write/extraction cost、storage growth、runtime injection——並寫明「誰付帳」
  (哪個既有注入同步瘦身)。新增行一律「有才出現」並有字數上限。「我才一行啦」不是理由:
  一人一行,最後湊成一頁。(記憶優化案,小踢採納為通則)
- **衍生資料不得自我放大。** digest、實體卡、檢索紀錄都是同一來源的投影,不是新證據;
  系統自己的檢索行為不得回頭增加該記憶的權重(retrieval 不訓練自己的排序器)。
  自查教訓:anchor 的 Hebbian 預設對系統檢索也強化,迴圈已在生產跑了幾個月才被指出。(記憶優化案 R6)
- **測試進程也是「施工者身在系統之內」(第 6 種形狀)。** 測試載入的模型跟生產服務搶同一塊記憶體:
  2GB 機器上 4 份 embedder 同時載入,load 衝 26、三個 tunnel 全 502、OOM killer 殺掉 anchor 本體,
  失聯的還是施工者自己的手。對策:測試用 stub 不載真模型;要載就先看 free、`systemd-run -p MemoryMax=`
  一次一份序列跑,超額死測試不死服務。(TICKET-Q 2026-09-21)
- **驗證未完成前,生產目錄不得處於「重啟即上新碼」的狀態。** TICKET-Q 的 OOM 只是扣扳機:
  真正的洞是新碼已直接寫進生產工作樹,任何意外重啟(OOM/needrestart/斷電)都等於未經批准的部署。
  正解:在 staging 目錄改與測,綠了才原子換入生產;或最少讓服務 pin 在舊碼直到放行。(TICKET-Q 覆核)
...
## 五、與平台共居（施工的系統住在別人的平台上）

- **兩套上限機制並存時，必須明確誰先觸發，並留跑道。** 換窗線 170k、官方壓縮線 ~165k
  ——「我們的線」設在「平台的線」之後，結構性必輸，四次成功全是運氣。
  我們的線一律設在平台的線之前並留餘裕；平台的線會隨版本移動，升級 SDK/CLI 後重驗這場賽跑。(INCIDENT_20260924)
- **靜默的逃生門本身要有警報。** auto-compact 把 cn 的時間線壓壞、全靠 Owner 語感發現。
  凡是「平台替我們兜底」的機制（自動壓縮、自動重啟、自動降級），觸發時必須有人知道。(INCIDENT_20260924，TICKET-T)
