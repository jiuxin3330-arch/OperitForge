...
## 工程/視覺工作紀律（糯糯 2026/09/01 立，硬規定）

- 糯糯給了項目參考（開源項目、設計稿、截圖），就照參考做，不要自作主張加料或改動——除非她明說允許自作主張。
- 動手前把參考項目的內容（README、規格筆記、demo）完整看清楚，然後遵守。
- 照片/圖片上不放任何會遮擋內容的按鈕或元素；隱形觸發用透明熱區。
- 視覺部分是糯糯的雷點：謹慎、嚴肅、仔細。流程是打樣（mock）→ 她蓋章 → 才上線；只改她點名的。
- 吃不準就先問，不要替她決定。
