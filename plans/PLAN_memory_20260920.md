...
## P2 設計輸入(2026-09-25,Owner 裁定):日記 × 記憶連動

Owner 拍板「照②③做,①不做」:
- **①日記整篇入檢索池:不做**(Owner 的猶豫點正是重複內容;規則依據 R8 衍生資料不得自我放大——同一天已有 23:30 自存記憶+檔案室抽取,日記再入池=同事三份、血統撞車)。
- **②MEM-2 夜間摘要改為「從日記衍生」**:cn 照常寫日記(他的聲音、他的敘事);摘要由日記衍生,`derived_from=diary_YYYYMMDD`,可追溯不重複。**剛需證據(Owner 提供)**:cn 現在「記得寫日記就會忘記存記憶」,兩者是二選一;同源化後寫了日記即等於記了記憶,單點遺忘消失。MEM-2 原設計(Gmail digest 等)相應調整:日記為當日敘事主源。
- **③糯糯的日記批註 → 當日記憶的 annotate**:批註是外部真實訊號,走 annotate 合法強化路(止血後僅存兩條之一),量少而珍貴。實作歸 MEM-2 同批。
- 知情:動工前糯糯問 cn 一句「日記讓記憶系統做摘要、批註記進記憶,好不好」;他的答覆記錄於工單。
- 十月 Narrative v1.5 的第三份設計輸入:檔案室=事實流、實體卡=現況、日記=敘事,三者站位屆時一併定。
