 ...
 * 改版輪 S1(規劃書 v2 蓋章):三區塊=檔案櫃抽屜,預設全關;把手標籤牌帶計數;
 * 待審紅標籤凸出/縮回。抽屜只是純 UI 包裝,資料流與 api 呼叫不變。
 */
  ...
  return (
    <div className="nest-archive">
      ...
      <section className={`nest-drawer${drawerOpen.states ? " is-open" : ""}`}>
        ...
        >
          <span className="nest-drawer-plate">
            <span className="nest-drawer-plate-name">現況登記</span>
            <span className="nest-drawer-plate-count">{states.length}</span>
          </span>
          <span className="nest-drawer-groove" aria-hidden="true" />
          <svg className="nest-drawer-arrow" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            <path d="M6 9l6 6l6 -6" />
          </svg>
        </button>
      ...
      <section className={`nest-drawer${drawerOpen.events ? " is-open" : ""}`}>
        ...
        >
          <span className="nest-drawer-plate">
            <span className="nest-drawer-plate-name">{searchActive ? "搜尋結果" : "最近事件"}</span>
            <span className="nest-drawer-plate-count">{eventsToShow.length}</span>
          </span>
          <span className="nest-drawer-groove" aria-hidden="true" />
          <svg className="nest-drawer-arrow" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            <path d="M6 9l6 6l6 -6" />
      ...
      <section className={`nest-drawer${drawerOpen.proposals ? " is-open" : ""}`}>
        <span
          className={`nest-drawer-tab${proposals.length > 0 ? " is-out" : ""}`}
        ...
        >
          <span className="nest-drawer-plate">
            <span className="nest-drawer-plate-name">待審提案</span>
            <span className="nest-drawer-plate-count">{proposals.length}</span>
          </span>
          <span className="nest-drawer-groove" aria-hidden="true" />
          <svg className="nest-drawer-arrow" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            <path d="M6 9l6 6l6 -6" />
